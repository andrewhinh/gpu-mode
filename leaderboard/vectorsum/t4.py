...
__global__ void vectorSumOptimized_v6_final_kernel(const float* __restrict__ input,
...
__global__ void finalReductionKernel_v6_final(const float* __restrict__ workspace,
...
torch::Tensor vectorSum(torch::Tensor image) {
    int n = image.numel();
    auto result = torch::zeros({}, image.options().dtype(torch::kFloat));
    
    int blockSize = 256;
    int gridSize = cdiv(n, blockSize * 16);
    if (gridSize < 1) gridSize = 1;
    
    auto workspace_tensor = torch::empty({gridSize}, image.options().dtype(torch::kFloat));
    
    dim3 block(blockSize);
    dim3 grid(gridSize);
    
    vectorSumOptimized_v6_final_kernel<<<grid, block>>>(
    ...
    dim3 block2(256);
    dim3 grid2(1);
    finalReductionKernel_v6_final<<<grid2, block2>>>(
        workspace_tensor.data_ptr<float>(),
...
"""
cpp_source = "torch::Tensor vectorSum(torch::Tensor image);"
