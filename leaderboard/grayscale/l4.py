...
cuda_source = """
// Helper to ensure proper device function inlining
#define DEVICE_INLINE __device__ __forceinline__

// Cast shared memory pointer to unsigned int for cp.async
DEVICE_INLINE unsigned cast_smem_ptr_to_uint(const void* ptr) {
    unsigned ret;
    asm volatile("{ .reg .u64 cast_smem_ptr_to_uint_temp; cvta.to.shared.u64 cast_smem_ptr_to_uint_temp, %1; cvt.u32.u64 %0, cast_smem_ptr_to_uint_temp; }" : "=r"(ret) : "l"(ptr));
    return ret;
}
...
void grayscale_kernel(
    ...
    // Get pointer to this warp's section of shared memory
    float* warp_inputs = block_inputs + warp_offset * 3;

    // Load RGB data from global to shared memory using cp.async
    {
        ...
        for (int i = 0; i < num_loads; ++i) {
            asm volatile(
                ...
                :
                : "r"(cast_smem_ptr_to_uint(&dst[i * warp_size])),
                  "l"(&src[i * warp_size]),
                  "n"(sizeof(load_type))
            );
    ...
    // Process RGB data in chunks and calculate grayscale
    constexpr int K = sizeof(load_type) / sizeof(float);

    unroll<0, num_stores>([&]<int i>() {
        constexpr int waits = num_stores - 1 - i;
        if constexpr (waits == 0) {
        ...
        // Store grayscale results back to global memory
